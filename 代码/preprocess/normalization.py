...
def clean_gadget(gadget: List[str]):  # 数组元素以代码行为单位
    ...
    return cleaned_gadget  # 以代码行为单位返回规范化后的代码


def main():
    file_path = './dataset/Vul_dataset'           # 漏洞文件路径
    pred_path = './dataset/normalized_data'       # 正则化后文件保存路径
    ...
    for dirname in tqdm.tqdm(os.listdir(file_path)):
        for file_name in os.listdir(file_path+'/'+dirname):
            ...
            nor_code = clean_gadget(code)  # 对代码行进行规范化
