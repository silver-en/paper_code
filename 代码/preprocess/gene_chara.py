...
if __name__ == '__main__':
    ...
    info_path = "./dataset/adj/backward" # 文件保存路径
    record_txt = "./dataset/record(backward).txt"
    all_node_path = "./dataset/all_node_num(backward).pkl" # 统计切片节点个数
        ...
        for file in tqdm.tqdm(os.listdir(dot_path+'/'+dir)):
            ...
            if (dir+'/'+file) in rec_list.split('\n'):
                ...
                if os.path.exists(dot_file):
                    ...
                    if backward: # 如果切片没有节点，就用原图
                        sorted_nodes = sorted(backward, key=num_list.index)  # 按照dot文件中的编号进行排序
                    else:
                    ...
                    if not os.path.exists(info_path+'/'+dir):
                        os.makedirs(info_path+'/'+dir)

                    with open(record_txt,'a+') as f:  # 往记录本中写入已处理的bin文件
                        f.writelines(dir+'/'+file+'\n')

                    stroe_path = info_path+'/'+dir+'/'+file+'.pkl'
