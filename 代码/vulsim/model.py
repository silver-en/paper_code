...
class Vulsim(nn.Module):
    def __init__(self, batch, node_num, input_dim,output_dim, dropout):
        ...
        ## 序列
        self.gru = nn.GRU(input_size = input_dim,hidden_size = input_dim,batch_first=False,dropout=dropout,num_layers=3)  # GRU
...
def collect_data(data):  
    ...
    pair_data_list = []
    for func in data:
        token_vector = data[func]['tokens_vector']  # 结点向量
        CDG_adj = data[func]['CDG_adj']  # CDG
        DDG_adj = data[func]['DDG_adj']  # DDG
        PDG_adj = data[func]['PDG_adj']  # PDG
        label = data[func]['label'] # 标签
...
class Vulsim_detector():
    def __init__(self, node_num, input_dim, output_dim, epochs, batch_size, \
        ...
        self.best_f1 = 0
        self.write = SummaryWriter('./log_data/branch/cdg+s_pdg/')

        save_path = './model_data/branch/cdg+s_pdg/Vulsim.pkl'
